-- D2P Academy | Migration 055
-- Enrich student progress RPC with form flags; add post-test unlock for attendance/event end.
  ...
  select jsonb_build_object(
    'enrollments', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'postTestUnlocked', (
          e.status in ('attended', 'completed')
          or ev.end_at <= timezone('utc', now())
          or exists (
            select 1
            from public.enrollment_attendance ea
            where ea.enrollment_id = e.id
              and ea.status = 'present'::public.attendance_status
          )
        )
